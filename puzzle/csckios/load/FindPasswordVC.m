...
@property (nonatomic,strong) MBProgressHUD* MBManager;

@end

@implementation FindPasswordVC

-(MBProgressHUD*)MBManager{
    if (_MBManager) {
        _MBManager = [[MBProgressHUD alloc]init];
    }
    return _MBManager;
}
...
-(void)creatUI{
    ...
    [autoCodeLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.height.mas_offset(50);
        make.left.mas_offset((WIDE-300)/2.0);
        make.top.mas_equalTo(_phoneTF.mas_bottom).offset(20);
    }];
    ...
    [passwordLable mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.height.mas_offset(50);
        make.left.mas_offset((WIDE-300)/2.0);
        make.top.mas_equalTo(_authCodeTF.mas_bottom).offset(10);
    }];
    ...
    [_tipLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.mas_equalTo(passwordLineView.mas_bottom);
    }];
    
    _submitBt = [WWUI creatButtonWithHight:CGRectZero targ:self sel:@selector(submitAction) titleColor:[UIColor colorWithRed:212.0/255 green:184.0/255 blue:118.0/255 alpha:1.0] font:[UIFont systemFontOfSize:17] image:nil backGroundImage:nil title:@"提交更改"];
    [self.view addSubview:_submitBt];
    _submitBt.layer.cornerRadius = 25;
    _submitBt.layer.masksToBounds = YES;
    _submitBt.layer.borderColor = [UIColor colorWithRed:212.0/255 green:184.0/255 blue:118.0/255 alpha:1.0].CGColor;
    _submitBt.layer.borderWidth = 1;
